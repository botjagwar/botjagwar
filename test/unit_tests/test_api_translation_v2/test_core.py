...
class TestTranslationStaticMethods(unittest.TestCase):
    ...
    def test_add_wiktionary_credit(self):
        ...
        entries = [
            Entry(entry='cat', definitions=['chat'],
                  part_of_speech='ana', language='en',
                  additional_data=None),
            Entry(entry='dog', definitions=['chien'],
                  part_of_speech='ana', language='en',
    ...
    def test_generate_summary_from_added_entries(self):
        # Create some sample input entries
        entries = [
            Entry(entry='chat', part_of_speech='ana', definitions=['cat'], language='fr', additional_data=None),
            Entry(entry='chien', part_of_speech='ana', definitions=['dog'], language='fr', additional_data=None),
            Entry(entry='pomme', part_of_speech='ana', definitions=['apple'], language='fr', additional_data=None),
            Entry(entry='apple', part_of_speech='ana', definitions=['pomme'], language='en', additional_data=None),
            Entry(entry='book', part_of_speech='ana', definitions=['livre'], language='en', additional_data=None),
            Entry(entry='livre', part_of_speech='ana', definitions=['book'], language='fr', additional_data=None),
        ]
...
class TestGenerateSummary(unittest.TestCase):
    ...
    def test_ninja_mode_1_target_page_exists_content_greater_than_125_percent_of_old_content(self):
        self.config_mock.get.return_value = '1'
        result = self.obj.generate_summary(
            self.entries, self.target_page_mock, 2 * self.target_page_mock.get.return_value[::-1])

        self.assertEqual(result, 'nanitatra')

    def test_ninja_mode_1_target_page_exists_content_less_than_or_equal_to_125_percent_of_old_content(self):
        self.config_mock.get.return_value = '1'
        result = self.obj.generate_summary(
            self.entries, self.target_page_mock, self.target_page_mock.get.return_value[::-1])
    ...
    def test_ninja_mode_1_target_page_does_not_exist_and_content_less_than_or_equal_to_200(self):
        self.config_mock.get.return_value = '1'
        self.target_page_mock.exists.return_value = False
        result = self.obj.generate_summary(
    ...
    def test_ninja_mode_other_than_1(self):
        ...
        result = self.obj.generate_summary(self.entries, self.target_page_mock, self.content_mock)
        self.assertEqual(result, self.obj._generate_summary_from_added_entries(self.entries))
