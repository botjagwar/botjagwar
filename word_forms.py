...
from api.importer import (
    AdditionalDataImporter,
    AdditionalDataImporterError,
    backend as db_backend,
)
...
def get_malagasy_page_set(template=None):
    ...
    mg_category_name = (
        TEMPLATE_TO_MG_CATEGORY[template]
        + " amin'ny teny "
...
def perform_function_on_entry(function):
    def process():
        ...
        for word_page in working_set:
            word_page = RedisPage(
                RedisSite(working_language, "wiktionary"), word_page, offline=False
            )
            ...
            print("%d / %d (%2.2f%%)" % (counter, total, 100.0 * counter / total))
