...
class RevisionLister(object):
    ...
    def get_revisions(self, count=250, most_recent=False):
        if self.last_count == count and self.last_most_recent == most_recent and self.last_returned is not None:
        ...
        for revision in revisions:
            print(revision)
            returned.append((revision.timestamp, revision.revid, self.page.getOldVersion(revision.revid)))
...
def build_regression_model_on_stats_data(language, site,
                                         milestones=(1e4, 2e4, 3e4, 4e4, 5e4, 6e4, 7e4, 8e4, 9e4,
...
def predict_milestones(site_type='wiktionary'):
    ...
    with open(wiki_file, 'w') as f:
        f.write("Ity vinavina ity dia nohavaozina farany ny "
        ...
        for language in wl.getLangs(site_type):
            ...
            try:
                milestone_dates = build_regression_model_on_stats_data(language, site_type)
            except ValueError:
                continue
    ...
    with open('user_data/future-milestones.wiki', 'r') as f:
        ...
        page.put(wikitext, 'Rôbô : fanavaozana ny dingana sy ny daty hahatratrarana azy')
