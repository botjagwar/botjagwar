...
from api.dictionary.exceptions.http import (
    WordAlreadyExists,
    WordDoesNotExist,
    InvalidJsonReceived,
)
...
def word_with_definition_exists(session, word_, language, part_of_speech, definition):
    words = (
        session.query(Word)
        .filter_by(word=word_, language=language, part_of_speech=part_of_speech)
        .all()
    )
    if not words:
        return False
...
async def get_word_by_id(request) -> Response:
    ...
    objekt = session.query(Word).filter_by(id=request.match_info["word_id"]).one()
...
async def get_entry(request) -> Response:
    ...
    objects = (
        session.query(Word)
        .filter_by(
            word=request.match_info["word"], language=request.match_info["language"]
        )
        .all()
    )
...
async def edit_entry(request) -> Response:
    ...
    with session.no_autoflush:
        ...
        word = session.query(Word).filter_by(id=request.match_info["word_id"]).all()
