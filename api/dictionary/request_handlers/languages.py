...
async def add_language(request) -> Response:
    ...
    if (
        "iso_code" not in data
        or "malagasy_name" not in data
        or "english_name" not in data
    ):
...
async def get_language(request) -> Response:
    ...
    language_data = session.query(Language).filter(Language.iso_code == code).all()
...
async def edit_language(request) -> Response:
    ...
    if isinstance(data, str):
        data = json.loads(data)
    language_data = (
        session.query(Language).filter(Language.iso_code == data["iso_code"]).all()
    )
    if len(language_data) < 1:
        raise ElementDoesNotExist()
    else:
...
async def delete_language(request) -> Response:
    ...
    session.query(Language).filter(
        Language.iso_code == request.match_info["iso_code"]
    ).delete()
