...
class TenyMalagasyPickleImporter(object):
    ...
    def run(self):
        counter = 0
        to_reach = 0
        for page in get_pages_from_category(
            "mg", "Anarana iombonana amin'ny teny malagasy"
        ):
            counter += 1
            print(counter)
            if counter <= to_reach:
        ...
        for definition in definitions:
            try:
                for pos in ["ana", "mat", "mpam"]:
                    word_id = self.importer.get_word_id(
                        entry.entry, entry.language, pos
                    )
...
class EnWiktionaryAdditionalDataImporter(object):
    ...
    def upload_additional_data(
        self, word_id: int, additional_data_type: str, additional_data: str
    ):
        if isinstance(additional_data, list):
            ...
            if 400 <= response.status_code <= 599:
                print(f"{data}")
                print(
                    f"import {additional_data_type} to word_id {word_id} failed: HTTP {response.status_code}: {response.text}"
                )
                return False
    ...
    def process_page(self, page):
        ...
        for entry in processor.get_all_entries(
            ...
            for additional_data_type, additional_data in entry_as_dict[
                ...
                if word_id := self.get_word_id(entry):
                    ...
                    self.upload_additional_data(
                        word_id, additional_data_type, additional_data
                    )
...
def main():
    ...
    parser.add_argument(
        "--program", dest="program", action="store", help="Program name"
    )
    parser.add_argument(
        "--start-from-page",
        dest="start_page",
        action="store",
        help="Start from a certain page number.",
    )
