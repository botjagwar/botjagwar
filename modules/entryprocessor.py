...
class process_frwikt(wiktprocessor):
    ...
    def getall(self, keepNativeEntries=False):
        ...
        definition = ""
        count = 0
        for content in re.split('==[ ]?\{\{langue\|([a-z]+[-]?[a-z]+)\}\}[ ]?==', self.content):
            count += 1
            if (count-1) % 2:
                lang = content               
                continue
            ...
            if lang.strip()=='fr':
                ...
                print "\{\{S\|([a-z]+)\|%s" % lang
                frpos = re.match("\{\{S\|([a-z]+)\|%s" % lang, content)
                #print frpos
                if frpos is not None:
                    frpos = frpos.groups()[0]
                else:
                    continue
                pos = frpos
    ...
    def getall2(self, keepNativeEntries=False):
        """languges sections in a given page formatting: [(POS, lang, definition), ...]"""
        assert type(self.Page) is wikipedia.Page
        items=[]
        ...
        for lang in re.findall('\{\{S\|([a-z]+)\|([a-z]{2,3})',self.content):
            ...
            print lang
            # word DEFINITION Retrieving
            d1 = ct_content.find("'{{S|%s|%s"%lang)+len("{{S|%s|%s"%lang)
            d2 = ct_content.find("=={{langue|", d1)+1 or ct_content.find("== {{langue|", d1)+1
            d_ptr = ct_content.find("=={{langue|%s" % lang[1], d1)+1 \
                    or ct_content.find("== {{langue|%s" % lang[1], d1)+1

            print "=={{langue|%s" % lang[1], d_ptr
            if d2 > d1:
