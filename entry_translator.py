...
def _update_statistics(rc_bot):
    if not rc_bot.stats["edits"] % 5:
        ...
        log.debug(
            (
                "%d/%02d/%02d %02d:%02d:%02d > " % cttime,
                "Fanovana: %(edits)d; pejy voaforona: %(newentries)d; hadisoana: %(errors)d"
                % rc_bot.stats
                + " taha: fanovana %.1f/min" % (60.0 * (5 / rc_bot.chronometer)),
            )
        )
...
async def handle_wiktionary_page(request) -> Response:
    ...
    def task():
        ...
        queue_size += 1
        try:
            translations.process_wiktionary_wiki_page(
                page, custom_publish_function=publish
            )
